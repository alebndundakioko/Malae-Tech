    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 sm:mb-12">
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-[#7A816C] font-bold text-[10px] sm:text-xs uppercase tracking-[0.2em]">
            <Heart className="w-4 h-4 fill-[#7A816C]" />
            Welcome back
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight leading-tight">
            {getGreeting()}, <span className="text-[#7A816C]">{auth.currentUser?.displayName?.split(' ')[0] || 'Doctor'}</span>
          </h1>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={onNewReport}
            className="w-full sm:w-auto group relative bg-[#7A816C] text-white px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-bold shadow-xl shadow-[#7A816C]/20 hover:shadow-2xl hover:shadow-[#7A816C]/30 transition-all flex items-center justify-center gap-3 overflow-hidden"
          >
              ...
              {(['all', 'original', 'story'] as const).map((type) => (
                <button
                  ...
                  className={`px-3 sm:px-4 py-1.5 rounded-lg text-[9px] sm:text-[10px] font-bold uppercase tracking-wider transition-all ${
                    filterType === type 
                      ? 'bg-white text-[#7A816C] shadow-sm' 
          ...
          <div className="flex items-center gap-4">
            <div className="relative flex-1 lg:w-80">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl border border-slate-100 bg-slate-50/50 text-xs sm:text-sm focus:outline-none focus:ring-4 focus:ring-[#7A816C]/5 focus:border-[#7A816C] transition-all placeholder:text-slate-400"
              />
            </div>
          </div>
        </div>
        ...
        <div className="min-h-[400px]">
          {loading ? (
                  ...
                  >
                    <div className="flex items-start gap-6 flex-1">
                      <div className={`
                        ...
                        ${report.type === 'story' 
                          ? 'bg-[#7A816C]/10 text-[#7A816C] group-hover:bg-[#7A816C] group-hover:text-white' 
                      ...
                      <div className="space-y-2 flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-3">
                          <h3 className="text-lg font-bold text-slate-900 group-hover:text-[#7A816C] transition-colors truncate">
                            {report.title}
                          </h3>
                    ...
                    <div className="flex items-center gap-4 self-end md:self-center">
                      ...
                      <div className="w-10 h-10 rounded-full border border-slate-100 flex items-center justify-center text-slate-300 group-hover:border-[#7A816C] group-hover:text-[#7A816C] transition-all">
            ...
            <div className="py-32 text-center px-6">
              ...
              <button
                onClick={onNewReport}
                className="inline-flex items-center gap-2 text-[#7A816C] font-black uppercase tracking-widest text-xs hover:gap-4 transition-all"
              >
